...
Процедура ДесериализацияПрикладныхТиповНаСервере()
	...
	// Поиск документов, которые имеются в системе по полученным номерам
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	мзОбновленныеДокументы = Новый Массив;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для каждого СтрокаТовар Из СтрокаТЗДокументов.ТЧДокумента Цикл
		...
		обДок.Записать();
		тзДокументы.Удалить(СтрокаТЗДокументов);
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(тзДокументы) Тогда
		Для каждого Документ Из тзДокументы Цикл
			...
			обДок.Дата = Документ.Дата;
			АвторДокумента = НайтиАвтораДокумента(Документ.Автор);
			Если ТипЗнч(АвторДокумента) = Тип("СправочникСсылка.Сотрудники") Тогда
				обДок.Автор = АвторДокумента;
			Иначе
				обСотрудник = Справочники.Сотрудники.СоздатьЭлемент();
				...
				обДок.Автор = обСотрудник.Ссылка;
			КонецЕсли;
			обДок.Комментарий = Документ.Комментарий;
			обДок.Номер = Документ.Номер;
			
			// Заполнение табличной части
			Для каждого СтрокаТовар Из Документ.ТЧДокумента Цикл
				НоваяСтрокаВТЧ = обДок.Товары.Добавить();
				НоменклатураТЧ = НайтиНоменклатуруТЧ(СтрокаТовар.Номенклатура);
				Если ТипЗнч(НоменклатураТЧ) = Тип("СправочникСсылка.Номенклатура") Тогда
					НоваяСтрокаВТЧ.Номенклатура = НоменклатураТЧ;
				Иначе
					обНоменклатура = Справочники.Номенклатура.СоздатьЭлемент();
					обНоменклатура.Наименование = НоменклатураТЧ;	
					обНоменклатура.Записать();
					НоваяСтрокаВТЧ.Номенклатура = обНоменклатура.Ссылка;
				КонецЕсли;
				НоваяСтрокаВТЧ.Количество = СтрокаТовар.Количество;
				НоваяСтрокаВТЧ.Стоимость = СтрокаТовар.Стоимость;			
				НоваяСтрокаВТЧ.Цена = СтрокаТовар.Цена;
			КонецЦикла;
